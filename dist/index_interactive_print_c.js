(function (factory) {
  ...
  let sketch = function (p) {
    ...
    function createGrid(box, topside, leftside) {
      ...
      const apparatus2 = createApparatus(cell_w, cell_h, init_top, init_left);
      const apparatus3 = createApparatus(cell_w, cell_h, init_top, init_left);
      const apparatus4 = createApparatus(cell_w, cell_h, init_top, init_left);
      let grid = [];
      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          ...
          if (roll < i / rows) curr_app = roll2 < j / cols ? apparatus : apparatus2;
          else curr_app = roll2 < j / cols ? apparatus3 : apparatus4;

          const content = curr_app[0].map((app) => {
